...
import moe.shizuku.manager.utils.CrashReporter
...
class AboutSettingsFragment : BaseSettingsFragment() {
    ...
    override fun onCreateSettingsPreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        findPreference<Preference>("manual_report")?.apply {
            val hasLastCrash = moe.shizuku.manager.utils.CrashHandler.getLastCrashReport(context) != null
            if (hasLastCrash) {
                ...
                showDialog(
                    MaterialAlertDialogBuilder(context)
                        ...
                        .setPositiveButton(R.string.manual_report_button_github) { _, _ ->
                            ...
                            if (hasLastCrash) {
                                moe.shizuku.manager.utils.CrashHandler.clearLastCrash(context)
                            }
                        }
                        .setNeutralButton(R.string.manual_report_copied_dialog_share) { _, _ ->
                            moe.shizuku.manager.utils.CrashReporter.shareAsFile(context)
                            if (hasLastCrash) {
                                moe.shizuku.manager.utils.CrashHandler.clearLastCrash(context)
                            }
                        }
