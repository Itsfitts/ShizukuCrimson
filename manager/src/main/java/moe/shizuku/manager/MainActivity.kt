...
import moe.shizuku.manager.migration.MigrationHelper
...
class MainActivity : HomeActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        try {
            ...
            if (moe.shizuku.manager.utils.CrashHandler.getLastCrashReport(this) != null) {
    ...
    private fun checkAndRestoreBackup() {
        lifecycleScope.launch(Dispatchers.IO) {
            val backupFile = moe.shizuku.manager.update.UpdateInstaller.getBackupFile(this@MainActivity)
            if (backupFile.exists()) {
                try {
                    val json = backupFile.readText()
                    if (moe.shizuku.manager.utils.SettingsBackupManager.import(this@MainActivity, json)) {
        ...
        try {
            MaterialAlertDialogBuilder(this)
                ...
                .setPositiveButton(R.string.manual_report_button_github) { _, _ ->
                    moe.shizuku.manager.utils.CrashReporter.shareAsFile(this)
                    moe.shizuku.manager.utils.CrashHandler.clearLastCrash(this)
                }
