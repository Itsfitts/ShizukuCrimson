...
import moe.shizuku.manager.di.appModule
import moe.shizuku.manager.worker.RemoteDbSyncWorker
...
class ShizukuApplication : Application(), Configuration.Provider {
            ...
            SentryAndroid.init(this) { options ->
                ...
                options.setBeforeSend { event, _ ->
                    ...
                    try {
                        ...
                        if (moe.shizuku.manager.utils.EnvironmentUtils.isSamsung()) {
                            event.setTag("oneui_version", moe.shizuku.manager.utils.EnvironmentUtils.getOneUiVersion().toString())
                        }
                        if (moe.shizuku.manager.utils.EnvironmentUtils.isOppo() || moe.shizuku.manager.utils.EnvironmentUtils.isOnePlus()) {
                            event.setTag("coloros_version", moe.shizuku.manager.utils.EnvironmentUtils.getColorOsVersion())
                        }
                        if (moe.shizuku.manager.utils.EnvironmentUtils.isXiaomi()) {
                            event.setTag("hyperos_version", moe.shizuku.manager.utils.EnvironmentUtils.getHyperOsVersion())
                        }
                        if (moe.shizuku.manager.utils.EnvironmentUtils.isTCL()) {
    ...
    private fun initializeManagers() {
        ...
        if (ShizukuSettings.getWatchdog()) {
            ...
            moe.shizuku.manager.worker.WatchdogWorker.schedule(this)
        }
    ...
    override fun onCreate() {
        ...
        if (moe.shizuku.manager.security.SecurityGuard.isTampered()) {
        ...
        Thread.setDefaultUncaughtExceptionHandler(moe.shizuku.manager.utils.CrashHandler(this, defaultHandler))
        ...
        try {
            ...
            if (ShizukuSettings.isLiveActivityEnabled()) {
                startService(Intent(this, moe.shizuku.manager.service.ShizukuLiveService::class.java))
            }
        } catch (e: Throwable) {
